...
import type { IWordlistRepository } from '../interfaces/index.js';

export class SqliteWordlistRepository implements IWordlistRepository {
  ...
  findById(id: number): WordlistRow | undefined {
    return this.db.prepare('SELECT * FROM wordlists WHERE id = ?').get(id) as WordlistRow | undefined;
  }
  ...
  getActiveWordlist(userId: number): WordlistRow {
    ...
    return this.db.prepare('SELECT * FROM wordlists WHERE id = 1').get() as WordlistRow;
  }
  ...
  deleteWordlist(id: number): void {
    ...
    this.db.transaction(() => {
      ...
      this.db.prepare('DELETE FROM wordlists WHERE id = ?').run(id);
    })();
  }
  ...
  updateWord(wordId: number, data: {
    ...
    const current = this.db.prepare('SELECT * FROM wordlist_words WHERE id = ?').get(wordId) as WordlistWordRow | undefined;
    ...
    this.db.prepare("UPDATE wordlists SET updated_at = datetime('now') WHERE id = ?").run(current.wordlist_id);
  }

  deleteWord(wordId: number, wordlistId: number): void {
    this.db.transaction(() => {
      this.db.prepare('DELETE FROM wordlist_words WHERE id = ? AND wordlist_id = ?').run(wordId, wordlistId);
