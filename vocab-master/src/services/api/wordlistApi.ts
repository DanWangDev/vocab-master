...
export const wordlistApi = {
  async getWordlists(): Promise<{ wordlists: Wordlist[] }> {
    return baseApi.fetchWithAuth('/wordlists');
  },

  async getWordlist(id: number): Promise<Wordlist> {
    return baseApi.fetchWithAuth(`/wordlists/${id}`);
  },

  async getWordlistWords(id: number): Promise<{ words: WordlistWord[] }> {
    return baseApi.fetchWithAuth(`/wordlists/${id}/words`);
  },

  async getActiveWordlist(): Promise<{ wordlist: Wordlist; words: WordlistWord[] }> {
    return baseApi.fetchWithAuth('/wordlists/active');
  },
  ...
  async importWordlist(file: File, name: string, description?: string): Promise<ImportResult> {
    ...
    const response = await fetch(`${baseApi.getBaseUrl()}/wordlists/import`, {
      method: 'POST',
      headers,
      body: formData,
    });
  ...
  async addWordsToWordlist(
    ...
    return baseApi.fetchWithAuth(`/wordlists/${wordlistId}/words`, {
      method: 'POST',
  ...
  async updateWordInWordlist(
    ...
    return baseApi.fetchWithAuth(`/wordlists/${wordlistId}/words/${wordId}`, {
      method: 'PUT',
      body: JSON.stringify(data),
    });
  },
