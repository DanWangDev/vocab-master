...
export const groupApi = {
  async getGroups(): Promise<{ groups: GroupSummary[] }> {
    return baseApi.fetchWithAuth('/groups');
  },

  async getGroup(id: number): Promise<GroupDetail> {
    return baseApi.fetchWithAuth(`/groups/${id}`);
  },
  ...
  async deleteGroup(id: number): Promise<void> {
    await baseApi.fetchWithAuth(`/groups/${id}`, { method: 'DELETE' });
  },

  async joinGroup(joinCode: string): Promise<GroupDetail> {
    return baseApi.fetchWithAuth('/groups/join', {
      method: 'POST',
  ...
  async removeMember(groupId: number, userId: number): Promise<void> {
    await baseApi.fetchWithAuth(`/groups/${groupId}/members/${userId}`, { method: 'DELETE' });
  },

  async updateMemberRole(groupId: number, userId: number, role: 'admin' | 'member'): Promise<void> {
    await baseApi.fetchWithAuth(`/groups/${groupId}/members/${userId}/role`, {
      method: 'PATCH',
  ...
  async assignWordlist(groupId: number, wordlistId: number): Promise<void> {
    await baseApi.fetchWithAuth(`/groups/${groupId}/wordlists`, {
      method: 'POST',
  ...
  async unassignWordlist(groupId: number, wordlistId: number): Promise<void> {
    await baseApi.fetchWithAuth(`/groups/${groupId}/wordlists/${wordlistId}`, { method: 'DELETE' });
  },
};
